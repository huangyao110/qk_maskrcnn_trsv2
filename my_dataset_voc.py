...
class VOCInstances(Dataset):
    def __init__(self, voc_root, year="2012", txt_name: str = "train.txt", transforms=None):
        ...
        if "VOCdevkit" in voc_root:
            ...
            root = os.path.join(voc_root, "VOCdevkit", f"VOC{year}") # 我们走的是这一步
        assert os.path.exists(root), "path '{}' does not exist.".format(root)
        image_dir = os.path.join(root, 'JPEGImages')
        xml_dir = os.path.join(root, 'Annotations')
        mask_dir = os.path.join(root, 'SegmentationObject')

        txt_path = os.path.join(root, "ImageSets", "Segmentation", txt_name) #读取图片名称（这个.txt保存是图片名称）
        assert os.path.exists(txt_path), "file '{}' does not exist.".format(txt_path)
        with open(os.path.join(txt_path), "r") as f:
            file_names = [x.strip() for x in f.readlines() if len(x.strip()) > 0] # 按行读取

        # read class_indict
        json_file = 'pascal_voc_indices.json'
        assert os.path.exists(json_file), "{} file not exist.".format(json_file)
        with open(json_file, 'r') as f: #在read模型下打开
            idx2classes = json.load(f) # 这是
        ...
        for idx, (img_path, xml_path, mask_path) in enumerate(zip(images_path, xmls_path, masks_path)):
            ...
            # 读取SegmentationObject并检查'filename' = {str} '2007_000032.jpg'是否和bboxes信息数量一致
    ...
    def __getitem__(self, idx):
        ...
        target["masks"] = masks #变成了一个3d的tensor
...
if __name__ == '__main__':
    print(os.getcwd())
    dataset = VOCInstances(voc_root=r"C:\Users\16477\Desktop\qk-dl\mask_rcnn\data")
    print(len(dataset))
    d1 = dataset[0]
